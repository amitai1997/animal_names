...
def mock_responses():
    ...
    return {
        ...
        "server_error": mock.Mock(
            status_code=500,
            raise_for_status=mock.Mock(side_effect=requests.HTTPError("500 Server Error")),
        ),
        "timeout": mock.Mock(
...
def test_extract_image_url(mock_get_session, mock_responses):
    ...
    assert extract_image_url("https://en.wikipedia.org/wiki/Cat") == "https://example.com/cat.jpg"
    
    # Test article image extraction
    mock_session.get.return_value = mock_response_with_article_image
    assert extract_image_url("https://en.wikipedia.org/wiki/Dog") == "https://example.com/dog.jpg"
...
@mock.patch("src.downloader.get_session")
def test_fetch_with_retries_large_file(mock_get_session, mock_responses, temp_image_dir):
...
@mock.patch("src.downloader.get_session")
def test_fetch_with_retries_server_error(mock_get_session, mock_responses, temp_image_dir):
...
@mock.patch("src.downloader.get_session")
def test_fetch_with_retries_persistent_error(mock_get_session, mock_responses, temp_image_dir):
    ...
    # Mock sleep to avoid waiting
    with mock.patch("time.sleep"):
        assert not fetch_with_retries("https://example.com/all_retries_failed.jpg", dest_path)
...
def test_download_images(
    ...
    animals = {
        ...
        "elephantine": [Animal(name="Elephant", page_url="https://en.wikipedia.org/wiki/Elephant")],
        "tigrine": [Animal(name="Tiger", page_url="https://en.wikipedia.org/wiki/Tiger")],
    }
