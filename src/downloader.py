    ...
    for attempt in range(retries):
        try:
            ...
            if 500 <= response.status_code < 600:
                logger.warning(f"Server error ({response.status_code}) for {url}, attempt {attempt+1}/{retries}")
                continue
...
def download_images(
    ...
    logger.info(f"Starting download of {len(unique_animals)} unique animal images using {workers} workers")
    ...
    def download_animal_image(animal: Animal) -> tuple[str, Optional[str]]:
        ...
        logger.error(f"Failed to download image for {name} and no placeholder available")
        return name, None
    ...
    logger.info(f"Image download complete: {success_count}/{total_count} successful ({success_rate:.1f}%)")
